...
import { getSupportIdentity, isAuthorizedSupportOperator, listTickets } from "@/lib/support/support-hub";
...
export async function GET(request: Request) {
  ...
  try { return NextResponse.json({ tickets: await listTickets(identity, new URL(request.url).searchParams.get("queue") ?? "unassigned") }); }
