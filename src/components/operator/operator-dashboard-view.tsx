...
function getActionLabel(action?: OperatorTaskActionState) {
  switch (action) {
    case "pickup_started":
      return "Validarea ridicării a început";
    case "package_loaded":
      return "Colet loaded";
    case "dropoff_started":
      return "Livrare validation started";
    case "issue_reported":
      ...
      <CardContent className="grid gap-4 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline">Comanda #{formatOrderId(task.orderId)}</Badge>
              <StatusBadge
        ...
        <div className="grid gap-3 text-sm md:grid-cols-2">
          <div className="rounded-[calc(var(--radius)+0.25rem)] border border-border/80 bg-secondary/45 px-4 py-3">
            <p className="text-muted-foreground">Ridicare</p>
            <p className="mt-1 font-medium text-foreground">
              {task.pickupSummary}
            </p>
          </div>
          <div className="rounded-[calc(var(--radius)+0.25rem)] border border-border/80 bg-secondary/45 px-4 py-3">
            <p className="text-muted-foreground">Livrare</p>
            <p className="mt-1 font-medium text-foreground">
              {task.dropoffSummary}
            </p>
          </div>
  ...
  return (
    <section className="app-container flex flex-col gap-8">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <StatCard
          label="Validări ridicare"
          value={`${data.pickupValidations.length}`}
          hint="Punct de ridicares that still need readiness or loading validation."
      ...
      <section id="pads" className="grid scroll-mt-24 gap-5 xl:grid-cols-2">
        <SectionCard
          eyebrow="Validări ridicare"
          title="Puncte de ridicare în așteptare"
        ...
        <SectionCard
          eyebrow="Livrare Validations"
          title="Livrare points pending"
          description="Livrare validation keeps recipient handoff and pad readiness visible before completion."
        >
          <div className="grid gap-3">
